...
export default function Home() {
  ...
  const loadData = useCallback(async () => {
    ...
    const newUsers = (await usersRes.json()) as PartnerUser[];
    const newReferrals = referralsRes.ok ? (await referralsRes.json()) as Referral[] : [];
    setUsers(newUsers);
        ...
        <div className="mt-10">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {referrals.map((r) => (
                  <tr key={r.id} className={`border-b border-gray-200 ${r.status === "submitted" ? "bg-yellow-50" : ""}`}>
                    <td className="px-2 py-3">
                      {`${r.first_name} ${r.last_name}`.trim() || r.partner_email}
                    </td>
                    <td className="px-2 py-3">
                    ...
                    <td className="px-2 py-3">
                      {r.lead_name || r.lead_email ? (
                        <div>
                          ...
                          {r.lead_email ? <span className="ml-1 text-brand-gray-300">{r.lead_email}</span> : null}
                        </div>
                      ) : (
                        <span className="text-brand-gray-200">-</span>
                      )}
                    ...
                    <td className="px-2 py-3">
                      <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-semibold ${
                        ...
                        {r.status === "submitted" ? "Needs Review" : r.status === "pending" ? "Link Click" : r.status}
                      </span>
                    </td>
                    <td className="px-2 py-3 text-brand-gray-300">
                    ...
                    <td className="px-2 py-3">
                      {r.status === "submitted" ? (
                        <ReviewActions referral={r} onReviewed={() => { void loadData(); }} />
                      ) : null}
                    </td>
                  </tr>
                ))}
...
function ReviewActions({ referral, onReviewed }: { referral: Referral; onReviewed: () => void }) {
    ...
    <div className="min-w-[250px] rounded border border-gray-200 bg-white p-3">
      {referral.notes ? (
        <div className="mb-2 text-xs">
          <span className="font-medium text-brand-gray-400">Partner notes:</span>{" "}
          <span className="text-brand-gray-600">{referral.notes}</span>
        </div>
      ) : null}
      {referral.lead_phone ? (
        <div className="mb-2 text-xs text-brand-gray-400">Phone: {referral.lead_phone}</div>
      ) : null}
      <input
        type="text"
        value={adminNote}
